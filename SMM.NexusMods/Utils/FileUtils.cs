...
namespace SubnauticaModManager.Utils
{
	internal static class FileUtils
	{
		...
		{
#if SHARP_COMPRESS
			ExtractSharpCompress(from, to);
#else
			if (Path.GetExtension(from) == ".zip")
			{
				System.IO.Compression.ZipFile.ExtractToDirectory(from, to);
			}
			else if (Path.GetExtension(from) == ".7z")
			{
		...
		private static void ExtractSharpCompress(string from, string to)
		{
			IArchive archive = ArchiveFactory.Open(from);
